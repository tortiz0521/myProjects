...
int main()
{
	cout << "Select a test to run:\nTest 1 - Testing RemoveAt() functionality. Removes a keyword from a string.\nTest 2 - Testing RemoveHead()/Tail() functionality. Removes from the head and tail of a linked list.\
\nTest 3 - Testing RemoveAt() and RemoveHead()/Tail() functionality.\nType '0' to exit.\n\nTest Selected: " << endl;
	...
	while (testNum != 0) {
		...
		cout << "\nSelect test '1', '2', or '3', or exit with 0: ";
		cin >> testNum;
	}
...
void TestRemove()
{
	...
	string test = "Test RemoveMe to RemoveMe find RemoveMe all RemoveMe matching RemoveMe ";
	test.append("nodes RemoveMe completed RemoveMe with RemoveMe no");
	test.append("\"RemoveMe\" nodes RemoveMe remaining.");
	stringstream x(test);
	string tmp;
	while (getline(x, tmp, ' '))
		data.AddTail(tmp);
